...
`ONE IMAGE (AUTO)` is for segmenting the current image, while `ENTIRE FOLDER (AUTO)` is for segmenting all images in the current folder, with the `ENTIRE FOLDER (AUTO)` segmentation progress displayed in the terminal.  
