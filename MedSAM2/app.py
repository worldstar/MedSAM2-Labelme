...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        opennext_medsam2 = action(
            self.tr("Next Semi but Auto"),
            self.openNextImg_medsam2,
            self.tr("Use same mode( semi-auto / auto one img) to segment next image，if semi-auto then use original points to complete the segmentation."),
            "next",

        )
        opennext_semi = action(
            self.tr("Next Use Semi-auto"),
            self.openNextImg_semi,
            self.tr("Use semi-auto mode to segment next image, click the point yourself."),
            "next",
        ...
        medsam2_adc = action(  # 預留快捷按鍵，未啓用
            self.tr("&ADC"),
            lambda: self.medsam2(
                checkpoint_path=os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint_adc_0311-2.pt",
                globa_data_path=os.path.dirname(os.path.abspath(__file__)) + "/globa_data.json"),
            shortcuts["semi_med"],
            self.tr(
                "Semi-automatic segmentation using MedSAM2,\ncan perform ADC medical image segmentation"),
        )
        medsam2_dce = action(  # 預留快捷按鍵，未啓用
            self.tr("&DCE"),
            lambda: self.medsam2(
                checkpoint_path=os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint_dce_0316-1.pt",
                globa_data_path=os.path.dirname(os.path.abspath(__file__)) + "/globa_data.json"),
            shortcuts["semi_med"],
            self.tr(
                "Semi-automatic segmentation using MedSAM2,\ncan perform DCE medical image segmentation"),
        )
        medsam2_dwi = action(  # 預留快捷按鍵，未啓用
            self.tr("&DWI"),
            lambda: self.medsam2(
                checkpoint_path=os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint_dwi_0325.pt",
                globa_data_path=os.path.dirname(os.path.abspath(__file__)) + "/globa_data.json"),
            shortcuts["semi_med"],
            self.tr(
                "Semi-automatic segmentation using MedSAM2,\ncan perform DWI medical image segmentation"),
        )
        medsam2_pet = action(  # 預留快捷按鍵，未啓用
            self.tr("&PET"),
            lambda: self.medsam2(
                checkpoint_path=os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint-all-4.pt",
                globa_data_path=os.path.dirname(os.path.abspath(__file__)) + "/globa_data.json"),
            shortcuts["semi_med"],
            self.tr(
                "Semi-automatic segmentation using MedSAM2,\ncan perform PET medical image segmentation"),
        )
        ...
        self.actions = utils.struct(
            ...
            opennext_medsam2=opennext_medsam2,
            choose_medsam2_mode=choose_medsam2_mode,
    ...
    #################################################################專題新增
    def openNextImg_semi(self, _value=False, load=True):
        # 保留原本開啟下一張圖片邏輯
        keep_prev = self._config["keep_prev"]
        if QtWidgets.QApplication.keyboardModifiers() == (
                Qt.ControlModifier | Qt.ShiftModifier
        ...
        if self.filename is None:
            ...
            currIndex = self.imageList.index(self.filename)
            if currIndex + 1 < len(self.imageList):
                self.filename = self.imageList[currIndex + 1]
            else:
                self.filename = self.imageList[-1]
        ...
        mode_path = os.path.join(base_dir, "globa_mode_select.json")
        label_path = os.path.join(base_dir, "globa_label.json")
        globa_data_path = os.path.join(base_dir, "globa_data.json")

        # 讀取模式
        if not os.path.exists(mode_path):
            print("[INFO] 無 globa_mode_select.json，跳過分割")
            return

        with open(mode_path, "r") as f:
            selected_mode = json.load(f).get("selected_mode", None)

        if selected_mode not in ["semi_auto"]:
            print(f"[INFO] 模式 {selected_mode} 執行切換為半自動分割")
        ...
        if os.path.exists(label_path):
            try:
                with open(label_path, "r") as f:
                    label_name = json.load(f).get("label_name", "obj")
            except:
                pass

        folder_name = os.path.basename(os.path.dirname(self.imagePath)).upper()
        if label_name.strip().lower() == "default":
            label_name = folder_name

        # 判斷 checkpoint
        default_checkpoint = os.path.join(base_dir, "checkpoints/checkpoint-all-4.pt")
        folder_name = os.path.basename(os.path.dirname(self.imagePath)).upper()
        if label_name.upper() == "DEFAULT":
            label_name = folder_name

        if label_name.upper() == "HEART":
            checkpoint_path = os.path.join(base_dir, "DLC-HEART/checkpoint-h.pt")
            if not os.path.exists(checkpoint_path):
                print("[INFO] 未安裝 DLC-HEART，跳過此次執行")
                return
        else:
            checkpoint_map = {
                "ADC": os.path.join(base_dir, "checkpoints/checkpoint-adc.pt"),
                "DCE": os.path.join(base_dir, "checkpoints/checkpoint-dce.pt"),
                "DWI": os.path.join(base_dir, "checkpoints/checkpoint-dwi.pt"),
            }
            checkpoint_path = checkpoint_map.get(folder_name, default_checkpoint)

        # 執行對應模式
        if selected_mode == "semi_auto":
            print("[AUTO] semi_auto 模式開啓，請進行點擊..")
            self.medsam2(
                checkpoint_path=checkpoint_path,
                globa_data_path=globa_data_path,
                label_name=label_name
            )

    def openNextImg_medsam2(self, _value=False, load=True):
        keep_prev = self._config["keep_prev"]
        if QtWidgets.QApplication.keyboardModifiers() == (
                Qt.ControlModifier | Qt.ShiftModifier
        ):
        ...
        if not os.path.exists(mode_path):
            print("[INFO] 無 globa_mode_select.json，跳過")
            return

        with open(mode_path, "r") as f:
            selected_mode = json.load(f).get("selected_mode", None)

        if selected_mode not in ["semi_auto", "auto_one"]:
            print(f"[INFO] 模式 {selected_mode} 不執行自動分割")
            return
        ...
        # === auto_one 模式 ===
        if selected_mode == "auto_one":
            print("[AUTO] auto_one 模式：執行全自動分割")
            self.automedsam2(checkpoint_path=checkpoint_path, label_name=label_name)
            return

        # === semi_auto 模式：直接執行不開tkinter ===
        print("[AUTO] semi_auto 模式：使用舊點位自動分割")
        ...
        if os.path.exists(globa_data_path):
            with open(globa_data_path, "r") as f:
                data = json.load(f)
                old_input_point = data.get("old_input_point")
                old_input_label = data.get("old_input_label")

        if not old_input_point or not old_input_label:
            print("[WARN] 無舊點位資料，跳過分割")
            return

        input_point_array = np.array(old_input_point)
        input_label_array = np.array(old_input_label, dtype=int)
...
'''
    ...
    def on_button_click(self, mode, label_name):
        # 取得預設 checkpoint
        default_checkpoint = os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint-all-4.pt"
        path_ = default_checkpoint
        ...
        if label_name.upper() == "HEART":
            ...
            # 根據資料夾名稱決定 checkpoint
            checkpoint_map = {
                "ADC": os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint-adc.pt",
                "DCE": os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint-dce.pt",
                "DWI": os.path.dirname(os.path.abspath(__file__)) + "/checkpoints/checkpoint-dwi.pt",
            }

            # 若資料夾名稱存在於指定類型中，使用對應 checkpoint，否則使用預設
            path_ = checkpoint_map.get(folder_name, default_checkpoint)
            print(f"using checkpoint : {os.path.basename(path_)}")
        ...
        if mode == "auto_folder":
            ...
            self.medsam2(checkpoint_path=checkpoint_path,
                         globa_data_path=os.path.dirname(os.path.abspath(__file__)) + "/globa_data.json",
                         label_name=label_name)
    ###單影像全自動模式
    def automedsam2(self, _value=False, load=True,checkpoint_path=None, label_name=None):
        ...
        mask_generator = SAM2AutomaticMaskGenerator(sam2) #全自動化分割
        # 預測圖片
        opened_image = np.array(Image.open(self.imagePath).convert("RGB"))
        image_height, image_width = opened_image.shape[:2]
        target_height = image_height * 1  # 後門
        ...
        if hasattr(detections, 'mask') and detections.mask is not None:
            ...
            for idx, contour in enumerate(contours):
                ...
                # **檢查是否有點在target height之下**
                is_in_bottom_third = any(y >= target_height for _, y in points)

                if not is_in_bottom_third:  # **只有不在target height下的多邊形才保留**
                    shape = {
                        "label": f"{label_name}_{idx + 1}",
                        "points": points,
                        "group_id": None,
