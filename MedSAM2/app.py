...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        medsam2_all = action(
            self.tr("&Semi\n(Click)"),
            lambda: self.medsam2(checkpoint_path="D:/anaconda3/envs/labelme/Lib/site-packages/labelme/checkpoints/checkpoint-all.pt",
                                 globa_data_path="D:/anaconda3/envs/labelme/Lib/site-packages/labelme/checkpoints/global_data.json"),
            shortcuts["semi_med"],
            "med-ai",
    ...
    def choose_medsam2(self):
        ...
        modes = {
            "ENTIRE FOLDER (AUTO)": ("auto_folder", "D:/anaconda3/envs/labelme/Lib/site-packages/labelme/icons/med-ai.PNG"),
            "ONE IMAGE (AUTO)": ("auto_one", "D:/anaconda3/envs/labelme/Lib/site-packages/labelme/icons/med-ai1.PNG"),
            "BATCH TASK (CLICK)": ("semi_n", "D:/anaconda3/envs/labelme/Lib/site-packages/labelme/icons/med-ai2.PNG"),
            "SEMI-AUTO (CLICK)": ("semi_auto", "D:/anaconda3/envs/labelme/Lib/site-packages/labelme/icons/med-ai3.PNG"),
        }
        
        buttons = {}
        for mode, (mode_value, icon_path) in modes.items():
        ...
        def on_confirm():
            if selected_mode[0]:
                dialog.accept()
                self.on_button_click(selected_mode[0])
    ...
    def on_button_click(self, mode):
        path_ = "D:/anaconda3/envs/labelme/Lib/site-packages/labelme/checkpoints/checkpoint-all-3.pt"
        checkpoint_paths = {
            "auto_folder": path_,
            "auto_one": path_,
            "semi_n": path_,
        ...
        if mode == "auto_folder":
            self.automedsam2_folder(checkpoint_path=checkpoint_path)
        elif mode == "auto_one":
            self.automedsam2(checkpoint_path=checkpoint_path)
        elif mode == "semi_n":
            self.medsam2_n(checkpoint_path=checkpoint_path)
        elif mode == "semi_auto":
            self.medsam2(checkpoint_path=checkpoint_path, globa_data_path="D:/anaconda3/envs/labelme/Lib/site-packages/labelme/global_data.json")
    ...
    def automedsam2(self, _value=False, load=True,checkpoint_path=None):
        ...
        target_height = image_height * 1/10  # 計算圖像的中線
    ...
    def automedsam2_folder(self, checkpoint_path=None):
        """
        ...
        for file in image_files:
            ...
            for shape in json_data.get("shapes", []):
                ...
                if area != 0:
                    ...
                    break
            if non_zero_area_files:
                print("\nTumor found in:")
                for filename in non_zero_area_files:
                    print(filename)
            else:
    ...
    def medsam2(self, checkpoint_path=None, globa_data_path=None):
        # 設定設備
        if torch.cuda.is_available():
            device = torch.device("cuda")
        elif torch.backends.mps.is_available():
    ...
    def medsam2_n(self, checkpoint_path=None):
        import copy
        ...
        def process_batch():
            ...
            if masks is not None and len(masks) > 0:
                ...
                print(f"此批次處理完成，新增 {len(masks)} 個 mask。")
            else:
                print("此批次未產生有效 mask。")
        ...
        btn_batch = tk.Button(btn_frame, text="分批處理（ESC）", command=process_batch, width=20)
